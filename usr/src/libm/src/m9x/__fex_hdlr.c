...
# endif
#else
#error Fex handler not supported on this platform
#endif
...
static pthread_key_t handlers_key;
static pthread_mutex_t handlers_key_lock = PTHREAD_MUTEX_INITIALIZER;

static struct sigaction oact;
static pthread_mutex_t hdlr_lock = PTHREAD_MUTEX_INITIALIZER;
	...
	struct __libm_mt_fex_sync_data *thr_env)
{
	...
	case __libm_mt_fex_start_master:
		...
		pthread_mutex_unlock(&thr_env->lock);
		break;
		...
		if ((int)inst.op & SIMD) {
			...
			*ap = 0;
			...
			if ((int)inst.op & INTREG) {
				/* set MMX mode */
				fpstate->sv_env.en_sw &= ~0x3800;
				fpstate->sv_env.en_tw = 0;
			}
		} else {
...
update_state:
	...
	*ap = accrued & i;
	fpstate->sv_env.en_sw &= ~0x3d;
	fpstate->sv_env.en_sw |= (accrued & ~i);
	fpstate->sv_env.en_cw |= 0x3d;
	fpstate->sv_env.en_cw &= ~i;
	if (test_sse_hw) {
		fpstate->sv_env.en_mxcsr &= ~0x3d;
		fpstate->sv_env.en_mxcsr |= 0x1e80 | (accrued & ~i);
		fpstate->sv_env.en_mxcsr &= ~(i << 7);
	}
	return;
...
__fex_get_thr_handlers()
{
	...
	if (pthread_main_np()) {
		...
		if (!ptr) {
			...
			if (pthread_setspecific(handlers_key, ptr) != 0) {
				(void)free(ptr);
				return NULL;
			}
			/* initialize to FEX_NOHANDLER if trap is enabled,
...
__fex_update_te()
{
	...
	if (!hdlr_installed && te) {
		act.sa_sigaction = (void *)__fex_hdlr;
